...
$bidang = 'Seksi Ketentraman dan Ketertiban Umum';
$nomorDokumen = '';
$jenisDokumen = '';
$tahunDokumen = '';
$namaDokumen = '';
...
if (isset($_POST['submit'])) {
    ...
    $target_dir = "assets/dokumen/seksi_ketentraman_dan_ketertiban_umum/";
    ...
    if (!$error && (!isset($_FILES['fileDokumen']) || $_FILES['fileDokumen']['error'] == UPLOAD_ERR_NO_FILE)) {
        ...
        $file_name_new = bin2hex(random_bytes(16)) . '-seksi_ketentraman_dan_ketertiban_umum.' . $file_type;
            ...
            if (move_uploaded_file($_FILES['fileDokumen']['tmp_name'], $target_file)) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    echo "<script>
                        ...
                        window.location.href='?req=bidang&pages=ketentraman';
                    </script>";
                    exit;
                } else {
                    unlink($target_file);
...
<div class="col-12 grid-margin stretch-card">
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Tambah Dokumen Seksi Ketentraman dan Ketertiban Umum</h4>
            <p class="card-description">Formulir untuk menambahkan dokumen baru ke Seksi Ketentraman dan Ketertiban Umum.</p>
            <?php if ($error): ?>
            ...
            <form class="forms-sample" method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>Upload File Dokumen (PDF, maks 5MB)</label>
                    <input type="file" name="fileDokumen" class="file-upload-default" id="fileDokumen" accept=".pdf" required>
                    <div class="input-group col-xs-12">
                        <input type="text" class="form-control file-upload-info" disabled placeholder="Pilih dokumen PDF">
                        <span class="input-group-append">
                            <button class="file-upload-browse btn btn-primary" type="button">Browse</button>
                        </span>
                    </div>
                    <small class="form-text text-muted">Hanya file PDF, maksimum 5MB.</small>
                </div>
