...
$bidang = 'Sub Bagian Perencanaan Evaluasi dan Keuangan';
$nomorDokumen = '';
$jenisDokumen = '';
$tahunDokumen = '';
$namaDokumen = '';
...
if (isset($_POST['submit'])) {
    ...
    $target_dir = "assets/dokument/sub_bagian_perencanaan_evaluasi_dan_keuangan/";
    if (!is_dir($target_dir)) {
        mkdir($target_dir, 0777, true);
    }

    if (!isset($_FILES['fileDokumen']) || $_FILES['fileDokumen']['error'] == UPLOAD_ERR_NO_FILE) {
        $error = 'File dokumen harus dipilih!';
    } else {
        $file_name = basename($_FILES['fileDokumen']['name']);
        $file_type = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
        $file_size = $_FILES['fileDokumen']['size'];
        $file_name_new = bin2hex(random_bytes(16)) . '-sub_bagian_perencanaan_evaluasi_dan_keuangan.' . $file_type;
            ...
            if (move_uploaded_file($_FILES['fileDokumen']['tmp_name'], $target_file)) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    echo "<script>
                        ...
                        window.location.href='?req=bidang&pages=perencanaan';
                    </script>";
                    exit;
                } else {
                    unlink($target_file);
                ...
                $error = 'Gagal mengunggah file! Periksa izin folder.';
            }
        }
    }
}
...
<div class="col-12 grid-margin stretch-card">
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Tambah Dokumen Sub Bagian Perencanaan Evaluasi dan Keuangan</h4>
            <p class="card-description">Formulir untuk menambahkan dokumen baru ke Sub Bagian Perencanaan Evaluasi dan Keuangan.</p>
            <?php if ($error): ?>
